...
在任意值上允许访问任何 属性 和 方法。 可以认为，声明一个变量为任意值之后，对它的任何操作，返回的内容的类型都是任意值。 
...
当然，我们一般不会在 ts 文件中使用 `export = foo` 来导出模块，而是在[写（符合 commonjs 规范的）第三方库的声明文件](https://github.com/xcatliu/typescript-tutorial/tree/6dc92acd3e00418984693b83d966994fcc381e33/basics/declaration-files/README.md#export-1)时，才会用到 `export = foo` 来导出类型。
