...
- vue.use的思路，为什么use以后就能全局用
  ...
  - 添加一个或多个全局组件。
  - 通过全局混入来添加一些组件选项。
  - 添加Vue实例方法，通过在其原型链上添加方法。

  2. `Vue.use()`的工作原理

  调用`Vue.use(Plugin)`时，会执行以下操作：

  - 检查是否已安装，如果已安装，则不会重复执行。
  - 如果插件是一个对象，它会调用插件的`install`方法。
  - 如果插件是一个函数，它将直接被调用。
  - `Vue.use`通常接受额外的选项参数，这些参数会传递给`install`方法或函数插件。

  3. 全局可用的原因

  - 当插件被安装时，它可以在内部对Vue进行各种操作（如添加全局组件、混入等），这些操作会影响到Vue的每个实例。
  - 由于Vue实例是共享同一个原型链的，所以一旦在原型链上添加了方法或属性，这些方法或属性在所有Vue实例中都是可用的。
  - 这意味着，无论你在应用的哪个部分创建一个新的Vue实例，这些全局方法、属性、组件等都是可访问的。

  结论

  `Vue.use()`是Vue的一个强大功能，它允许插件作者向Vue添加全新的功能，而开发者则可以通过简单地调用`Vue.use()`来让这些功能在整个应用中可用。这大大提升了Vue生态系统的灵活性和模块化。

  `Vue.use()`的作用确实是将插件添加到Vue应用中，但并不总是通过添加到Vue的原型链上来实现全局使用。具体来说：

  1. **添加到原型链**：对于那些需要在每个Vue实例中使用的功能，如实例方法，插件确实会被添加到Vue的原型链上。这意味着这些方法或属性将在所有Vue实例中可用。例如，如果插件向`Vue.prototype`添加了一个方法，那么这个方法会在所有Vue实例中可用。
  2. **其他全局添加方式**：但`Vue.use()`也可以以其他方式使插件功能全局可用。例如：
     - 注册全局组件：使得这些组件可以在任何新创建的Vue实例的模板中使用。
     - 添加全局混入（Mixins）：影响到每个Vue实例的行为。
     - 添加全局指令或过滤器。
     - 提供全局方法或属性，这些方法或属性不是添加到每个实例上，而是可以通过Vue全局对象访问。
  3. **安装函数**：如果插件是一个对象，`Vue.use()`会调用其`install`方法。这个`install`方法负责执行所有必要的步骤来添加插件的功能，无论是添加到原型链，还是以其他方式全局添加。
  4. **插件本身作为函数**：如果插件本身是一个函数，则该函数本身被视为`install`方法。

  总的来说，`Vue.use()`的核心作用是执行插件的安装逻辑，而这个逻辑可能包括但不限于将功能添加到Vue的原型链上。这种灵活性使得插件可以以多种方式扩展Vue的功能。
